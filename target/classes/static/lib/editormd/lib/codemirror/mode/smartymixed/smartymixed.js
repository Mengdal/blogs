...
CodeMirror.defineMode("smartymixed", function(config) {
  ...
  if (config.hasOwnProperty("leftDemdhter")) {
    settings.leftDemdhter = config.leftDemdhter;
  }
  if (config.hasOwnProperty("rightDemdhter")) {
    settings.rightDemdhter = config.rightDemdhter;
  }

  function reEsc(str) { return str.replace(/[^\s\w]/g, "\\$&"); }

  var reLeft = reEsc(settings.leftDemdhter), reRight = reEsc(settings.rightDemdhter);
  var regs = {
  ...
  var parsers = {
    html: function(stream, state) {
      ...
      if (!state.inLiteral && stream.match(regs.htmlHasLeftDelimeter, false) && htmlTagName === null) {
        ...
        return helpers.maybeBackup(stream, settings.leftDemdhter, smartyMode.token(stream, state.localState));
      } else if (!state.inLiteral && stream.match(settings.leftDemdhter, false)) {
        state.tokenize = parsers.smarty;
        state.localMode = smartyMode;
        state.localState = smartyMode.startState(htmlMixedMode.indent(state.htmlMixedState, ""));
        return helpers.maybeBackup(stream, settings.leftDemdhter, smartyMode.token(stream, state.localState));
    ...
    smarty: function(stream, state) {
      if (stream.match(settings.leftDemdhter, false)) {
        if (stream.match(regs.smartyComment, false)) {
          return helpers.chain(stream, state, parsers.inBlock("comment", "*" + settings.rightDemdhter));
        }
      } else if (stream.match(settings.rightDemdhter, false)) {
        stream.eat(settings.rightDemdhter);
      ...
      return helpers.maybeBackup(stream, settings.rightDemdhter, smartyMode.token(stream, state.localState));
    },
