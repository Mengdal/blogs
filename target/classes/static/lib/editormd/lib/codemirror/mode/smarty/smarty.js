...
CodeMirror.defineMode("smarty", function(config) {
  ...
  var parsers = {
    ...
    tokenizer: function(stream, state) {
      if (stream.match(settings.leftDemdhter, true)) {
        if (stream.eat("*")) {
          ...
          if (settings.smartyVersion === 3 && settings.leftDemdhter === "{" && (isEol || isFollowedByWhitespace)) {
            state.depth--;
            return null;
          } else {
    ...
    smarty: function(stream, state) {
      if (stream.match(settings.rightDemdhter, true)) {
